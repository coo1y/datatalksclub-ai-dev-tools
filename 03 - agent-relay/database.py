...
This module is intentionally the only place that knows about connection
pragmas and the writer-lock transaction strategy for whichever backend
``RELAY_DATABASE_URL``/``DATABASE_URL`` points at. SQLite serializes every
writer transaction with ``BEGIN IMMEDIATE`` (it has no ``FOR UPDATE SKIP
LOCKED``); PostgreSQL instead uses ordinary transactions plus targeted row
locks via :func:`lock_for_update`, so concurrent claims on different tasks
proceed in parallel instead of queuing behind a single writer lock.
"""
...
def lock_for_update(query: Any, *, skip_locked: bool = False) -> Any:
    """Lock the rows a query selects, on backends that support it.

    PostgreSQL locks the matched rows with ``SELECT ... FOR UPDATE`` (optionally
    ``SKIP LOCKED`` so concurrent claims on different tasks do not block each
    other). SQLite already serializes every writer transaction with ``BEGIN
    IMMEDIATE`` (see :func:`immediate_transaction`), so no per-row lock is
    needed, and SQLite does not support the clause anyway.
    """
...
def immediate_transaction() -> Generator[Session, None, None]:
    ...
    recovery or terminal submissions) across API processes, since SQLite has
    no row-level locking. On PostgreSQL, an ordinary transaction is enough;
    the functions that need exclusivity lock their specific rows with
    :func:`lock_for_update` instead of serializing every writer.
    """
...
def recover_expired_in_session(db: Session, now: datetime) -> int:
    ...
    for attempt in expired:
        task = db.scalar(lock_for_update(select(Task).where(Task.id == attempt.task_id)))
