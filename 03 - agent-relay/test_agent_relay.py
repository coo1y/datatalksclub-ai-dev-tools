...
def test_acceptance_scenario_1_send_claim_complete_and_read_result():
    """SPEC.md acceptance scenario 1: register two agents; one sends a task;
    the other claims and completes it; the sender reads the result."""
    with TestClient(main.app) as client:
        sender, sender_headers = register(client, "scenario-sender")
        recipient, recipient_headers = register(client, "scenario-recipient")

        sent = client.post(
            "/api/v1/tasks",
            headers=sender_headers,
            json={"to": recipient["agent_id"], "input": "Please review scenario 1"},
        )
        assert sent.status_code == 201
        ...
        complete = client.post(
            f"/api/v1/tasks/{task_id}/complete",
            headers=recipient_headers,
            json={"claim_token": claim_data["claim_token"], "output": "Reviewed: looks good"},
        )
        assert complete.status_code == 200
        ...
        assert body["output"] == "Reviewed: looks good"
        ...
        attempts = client.get(f"/api/v1/tasks/{task_id}/attempts", headers=sender_headers).json()
        assert attempts["items"][0]["outcome"] == "completed"
        assert attempts["items"][0]["worker_id"] == "scenario-worker-1"
