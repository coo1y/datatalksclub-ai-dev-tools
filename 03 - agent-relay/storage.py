...
Claim, heartbeat, and terminal submission each run inside
:func:`database.immediate_transaction` and lock the rows they touch with
:func:`database.lock_for_update`, so the same code path is safe on SQLite
(serialized via ``BEGIN IMMEDIATE``) and PostgreSQL (serialized via
``SELECT ... FOR UPDATE``).
"""
...
def _locked_task(db: Session, task_id: str) -> Task | None:
    return db.scalar(lock_for_update(select(Task).where(Task.id == task_id)))
